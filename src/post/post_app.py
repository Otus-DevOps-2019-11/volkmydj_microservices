...
class HttpTransport(BaseTransportHandler):
    ...
    def send(self, encoded_span):
        ...
        try:
            requests.post(
                ZIPKIN_URL, data=body, headers={"Content-Type": "application/x-thrift"}
            )
...
zipkin_transport = HttpTransport() if ZIPKIN_ENABLED else DummyTransport()
...
def init(app):
    ...
    app.post_read_db_seconds = prometheus_client.Histogram(
        "post_read_db_seconds", "Request DB time"
    )
    app.post_count = prometheus_client.Counter("post_count", "A counter of new posts")
    # database client connection
    app.db = MongoClient(POST_DATABASE_HOST, int(POST_DATABASE_PORT)).users_post.posts
...
def vote():
    try:
        ...
        log_event(
            "error", "request_error", "Bad input parameters. Reason: {}".format(str(e))
        )
        abort(400)
    try:
        ...
        app.db.update_one(
            {"_id": ObjectId(post_id)}, {"$set": {"votes": post["votes"]}}
        )
    except Exception as e:
        log_event(
            "error",
...
def add_post():
    ...
    try:
        app.db.insert(
            {"title": title, "link": link, "created_at": created_at, "votes": 0}
        )
    except Exception as e:
        log_event(
            "error",
...
def healthcheck():
    return http_healthcheck_handler(POST_DATABASE_HOST, POST_DATABASE_PORT, app.version)
...
def after_request(response):
    request_id = (
        request.headers["Request-Id"] if "Request-Id" in request.headers else None
    )
    log.info(
        "request",
        service="post",
